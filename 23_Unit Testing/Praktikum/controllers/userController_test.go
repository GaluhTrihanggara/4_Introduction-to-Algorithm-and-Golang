...
func TestGetUserDetailControllers(t *testing.T) {
	...
	rec = httptest.NewRecorder()
	c = e.NewContext(httptest.NewRequest(http.MethodGet, "/users/invalid", nil), rec)
	c.SetParamNames("id")
	c.SetParamValues("invalid")
	err := GetUserDetailControllers(c).(*echo.HTTPError)
	if assert.Error(t, err) {
		assert.Equal(t, http.StatusBadRequest, err.Code)
		assert.Contains(t, err.Message.(string), "invalid syntax")
	}
}
